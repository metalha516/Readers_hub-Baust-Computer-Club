...
(function () {
  ...
  function loadSheetViaJSONP() {
    return new Promise((resolve, reject) => {
      const callbackName = 'gvizCallback_art_' + Date.now() + '_' + Math.floor(Math.random() * 1000);
  ...
  async function loadData() {
    ...
    try {
      ...
      if (jsonpRows && jsonpRows.length > 0) {
        allArticles = normalizeArticles(jsonpRows);
        renderArticle();
        return;
      }
    } catch (err1) {
    ...
    try {
      ...
      if (proxyRows && proxyRows.length > 0) {
        allArticles = normalizeArticles(proxyRows);
        renderArticle();
        return;
      }
    } catch (err2) {
  ...
  function formatDateString(rawDateStr) {
    ...
    return str;
      ...
      let rawDate = item.Date || item.date || '2026-09-04';
